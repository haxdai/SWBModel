...
public class BaseNode extends BaseNodeBase {
	...
	public static String getOnParentVersion(BaseNode node, BaseNode parent) {
		SemanticObject nodeDefinition = BaseNode.getChildNodeDefinition(parent.getSemanticObject().getSemanticClass(),
				node.getName());
		...
		if (isLockable()) {
			if (lockOwner == null || lockOwner.trim().equals("")) {
				...
				if (lockOwnerValue == null) {
					...
					setPropertyInternal(propertyIsDeep, Boolean.toString(lockIsDeep));
				}
			}
		} else {
	...
	public boolean isMandatory() {
		boolean isMandatory = false;
		SemanticLiteral literal = getSemanticObject().getSemanticClass()
				.getRequiredProperty(CommonPropertiesforDefinition.jcr_mandatory);
		if (literal != null && literal.getString() != null) {
			isMandatory = Boolean.parseBoolean(literal.getString());
		}
		return isMandatory;
	 ...
	 *             the sWB exception
	 */
	private void savePropertyToDB(SemanticProperty property, String value) {
		if (value == null) {
			getSemanticObject().removeProperty(property);
		} else {
			getSemanticObject().setProperty(property, value);
	...
	public String getProperty(SemanticProperty property) throws SWBException {
		...
		if (property.isDataTypeProperty()) {
			...
			SWBException swbe = new SWBException(
					"The property is an object property and can not be transform to String");
			log.trace(swbe);
			throw swbe;
		}
		return getProperty;
	...
	public final Iterator<SemanticProperty> listSemanticProperties(SemanticClass clazz) {
		HashSet<SemanticProperty> propertiesToReturn = new HashSet<>();
	 ...
	 * @return true, if is autocreated
	 */
	public static final boolean isAutocreated(SemanticProperty property) {
		boolean isAutocreated = false;
			...
			if (name != null && name.equals(nameTocreate) && requiredType != null) {
				try {
					...
					if (clazzToCreate.equals(clazzAllowed) || clazzToCreate.isSubClass(clazzAllowed)) {
						if (nodeDefinition.getProperty(ChildNodeDefinition.jcr_sameNameSiblings) != null
								&& nodeDefinition.getProperty(ChildNodeDefinition.jcr_sameNameSiblings)
										.equals("true")) {
	...
	private final void addUUID() {
		if (isReferenceable() || isVersionable()) {
			SemanticProperty jcrUUID = Referenceable.jcr_uuid;
			String value = getPropertyInternal(jcrUUID, null);
			if (value == null) {
				setPropertyInternal(jcrUUID, UUID.randomUUID().toString());
			}
		}
	}
	...
	public final SemanticClass[] getMixInNodeTypes() {
		ArrayList<SemanticClass> classes = new ArrayList<>();
	 ...
	 *             the sWB exception
	 */
	private final SemanticLiteral getSemanticLiteral(SemanticClass clazz, SemanticProperty property) {
	 ...
	 *             the sWB exception
	 */
	private BaseNode addNodeToProperty(SemanticProperty property, SemanticClass clazz, String name, String path)
			throws SWBException {
		if (property.isObjectProperty()) {
			...
			if (value == null) {
				...
				if (clazzDomain.equals(clazz) || clazzDomain.isSuperClass(clazz)) {
					...
					throw new SWBException("The property has a diferent nodeType (" + clazzDomain.getPrefix() + ":"
							+ clazzDomain.getName() + ") of the NodeType(" + clazz.getPrefix() + ":" + clazz.getName()
							+ ") to create.");
				}
			}
	...
	private void doCopy(BaseNode targetNode, BaseNode destNode) throws SWBException {
		...
		while (nodes.hasNext()) {
			...
			BaseNode copyChild = destNode.createNodeBase(childNode.getName(),
					childNode.getSemanticObject().getSemanticClass());
		...
		while (properties.hasNext()) {
			...
			if (!isInternal(property)) {
				...
				if (property.isObjectProperty() && onParentVersion.equals(ONPARENTVERSION_COPY)) {
					copyChild.getSemanticObject().setObjectProperty(property,
							this.getSemanticObject().getObjectProperty(property));
				}
			}
		}
	}
			...
			while (properties.hasNext()) {
				...
				if (!isInternal(property)) {
					...
					if (property.isDataTypeProperty() && onParentVersion.equals(ONPARENTVERSION_COPY)) {
						...
						log.trace("Copiando propiedad " + property.getURI());
						frozenNode.getSemanticObject().setObjectProperty(property,
								this.getSemanticObject().getObjectProperty(property));
					} else {
	...
	private void initializeFrozenProperties(SemanticObject frozenNode) throws SWBException {
		SemanticProperty jcrFrozenUuid = FrozenNode.jcr_frozenUuid;
		SemanticProperty jcrFrozenPrimaryType = FrozenNode.jcr_frozenPrimaryType;
		SemanticProperty frozenMixinTypes = FrozenNode.jcr_frozenMixinTypes;
		String uudi = this.getUUID();
		frozenNode.setProperty(jcrFrozenUuid, uudi);
		frozenNode.setProperty(jcrFrozenPrimaryType, this.getPrimaryType());
	...
	private void createVersionHistoryNode() throws SWBException {
		if (isVersionable()) {
			...
			BaseNode historyNode = addNodeToProperty(property, VersionHistory.nt_VersionHistory, "jcr:versionHistory",
					path);
	 ...
	 *             the sWB exception
	 */
	public BaseNode[] getBaseSuccessors() {
		ArrayList<BaseNode> successors = new ArrayList<>();
	 ...
	 *             the sWB exception
	 */
	public BaseNode[] getBasePredecessors() {
		ArrayList<BaseNode> predecessors = new ArrayList<>();
	 ...
	 *             the sWB exception
	 */
	public BaseNode getHistoryNode() {
	...
	private BaseNode addVersionToHistoryNode() throws SWBException {
		...
		if (isVersionable()) {
			...
			if (versionHistory != null) {
				...
				// TODO: DEBE AGREGAR REFERENCIA A LAS PREDECEDORAS, PERO FALTA METODO PARA
				// AGREGAR
				for (BaseNode predecessor : predecessors) {
					predecessor.getSemanticObject().addObjectProperty(Version.jcr_successors,
							ntVersion.getSemanticObject());
					ntVersion.getSemanticObject().addObjectProperty(Version.jcr_predecessors,
							predecessor.getSemanticObject());
				}
				BaseNode ntFrozenNode = ntVersion.createNodeBase(JCR_FROZENNODE_NAME, FrozenNode.nt_FrozenNode);
	...
	public static SemanticObject[] getChildNodeDefinition(SemanticClass clazz) {
		ArrayList<SemanticObject> getChildNodeDefinition = new ArrayList<>();
	...
	public boolean canAddNode(SemanticClass clazzSurce, SemanticClass clazzDest) {
		...
		if (childDefinitions.length > 0) {
			for (SemanticObject childDefinition : childDefinitions) {
				String jcrRequiredType = childDefinition.getProperty(ChildNodeDefinition.jcr_requiredPrimaryTypes);
				if (jcrRequiredType != null) {
					try {
						SemanticClass classRequiredType = getSemanticClass(jcrRequiredType);
						if (classRequiredType != null) {
							if (classRequiredType.equals(clazzSurce) || clazzSurce.isSubClass(classRequiredType)) {
								canAddNode = true;
								break;
							}
						}
	 ...
	 *             the sWB exception
	 */
	private void addCreatedProperty() {
		if (isVersionNode()) {
			SemanticProperty jcrCreated = Traceable.jcr_created;
			if (jcrCreated != null) {
				String value = SWBUtils.TEXT.iso8601DateFormat(Calendar.getInstance().getTime());
				setPropertyInternal(jcrCreated, value);
			}
		}
	}
	...
	public static SemanticObject[] getPropertyDefinition(SemanticClass clazz) {
		ArrayList<SemanticObject> propertyDefinitions = new ArrayList<>();
	...
	public boolean isProtected() {
		boolean isProtected = false;
		Iterator<SemanticLiteral> literals = getSemanticObject().getSemanticClass()
				.listRequiredProperties(CommonPropertiesforDefinition.jcr_protected);
		while (literals.hasNext()) {
			SemanticLiteral literal = literals.next();
			if (literal != null && literal.getString() != null) {
	...
	public final BaseNode createNodeBase(String name, SemanticClass clazz) throws SWBException {
		if (canAddNode(clazz)) {
			...
			throw new SWBException(
					"The nodetype " + clazz.getPrefix() + ":" + clazz.getName() + " is not possible to add");
		}
	}
	 ...
	 *             the sWB exception
	 */
	private void addVersionableUuid() {
			...
			if (!existsProperty) {
				...
				try {
					...
					for (Field field : clazzJava.getFields()) {
						if (field.getType().equals(SemanticProperty.class) && Modifier.isPublic(field.getModifiers())
	...
	public SemanticProperty registerCustomProperty(String name, String type, SemanticClass clazz) throws SWBException {
		if (!existsProperty(name, clazz)) {
			...
			SemanticProperty prop = getSemanticObject().getModel().createSemanticProperty(uri,
					this.getSemanticObject().getSemanticClass(), SemanticVocabulary.OWL_DATATYPEPROPERTY, type);
			return prop;
		} else {
	...
	public SemanticProperty registerCustomObjectProperty(String name, String type, SemanticClass clazz)
			throws SWBException {
		if (!existsProperty(name, clazz)) {
			String uri = this.getURI().replaceAll("#", "/") + "#" + name;
			SemanticProperty prop = getSemanticObject().getModel().createSemanticProperty(uri,
					this.getSemanticObject().getSemanticClass(), SemanticVocabulary.OWL_OBJECTPROPERTY, type);
			return prop;
		} else {
			throw new SWBException("the property already exists");
		}
	...
	public BaseNode[] getVersions() throws SWBException {
		if (this.isVersionHistoryNode()) {
			ArrayList<BaseNode> versions = new ArrayList<>();
		...
		while (tpcit.hasNext()) {
			...
			if (tpc.getPrefix() != null) {
				...
				while (tppit.hasNext()) {
					...
					if (tpp.getPrefix() != null && !namespaces.containsKey(tpp.getPrefix())) {
