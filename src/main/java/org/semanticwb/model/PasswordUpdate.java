...
public class PasswordUpdate extends PasswordUpdateBase {
	...
	public String renderElement(HttpServletRequest request, SemanticObject obj, SemanticProperty prop, String propName,
		...
		return renderXHTML(request, obj, prop, propName, type, mode, lang);
	}
	...
	public void validate(HttpServletRequest request, SemanticObject obj, SemanticProperty prop, String propName)
		...
		if (SWBPlatform.getSecValues().getComplexity(repo) == 1
				&& (!password.matches("^.*(?=.*[a-zA-Z])(?=.*[0-9])().*$"))) {
			throw new FormValidateException("Password don't comply with security measures: simple");
		}
		if (SWBPlatform.getSecValues().getComplexity(repo) == 2
				&& (!password.matches("^.*(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[\\W])().*$"))) {
			throw new FormValidateException("Password don't comply with security measures: complex");
		}
		if (SWBPlatform.getSecValues().getComplexity(repo) == 3
				&& (!password.matches(SWBPlatform.getSecValues().getCustomExp(repo)))) {
