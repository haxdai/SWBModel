 ...
 * 
 * @deprecated Use FileUpload instead. This element uses Flash plugin to upload
 *             files in background, but Flash plugin is now not supported in
 *             major browsers.
 */
public class FlashImageUpload extends org.semanticwb.model.base.FlashImageUploadBase {
	...
	private void imgPrpcess(File dest) throws IOException {
		...
		if (isImgCrop()) {
			ImageResizer.resizeCrop(tmpFile, getImgMaxWidth(), getImgMaxHeight(), new File(path, name),
					name.substring(name.lastIndexOf(".") + 1));
		} else {
			ImageResizer.resize(tmpFile, getImgMaxWidth(), getImgMaxHeight(), true, new File(path, name),
					name.substring(name.lastIndexOf(".") + 1));
		}
		if (isImgThumbnail()) {
			if (isImgCrop()) {
				ImageResizer.resizeCrop(tmpFile, getImgThumbnailWidth(), getImgThumbnailHeight(),
						new File(path, "thmb_" + name), name.substring(name.lastIndexOf(".") + 1));
			} else {
				ImageResizer.resize(tmpFile, getImgThumbnailWidth(), getImgThumbnailHeight(), true,
						new File(path, "thmb_" + name), name.substring(name.lastIndexOf(".") + 1));
			}
		}
		tmpFile.delete();
