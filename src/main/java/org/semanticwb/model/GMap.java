...
public class GMap extends org.semanticwb.model.base.GMapBase {
	...
	public String renderElement(HttpServletRequest request, SemanticObject obj, SemanticProperty prop, String propName,
		...
		if (obj == null)
			...
			nombre = ob.getDisplayName(lang);
		int step = ob.getIntProperty(prop3);
		if (0 == step)
			step = 10;
		...
		try {
			...
			map = (GMap) ob.getObjectProperty(prop).createGenericInstance();
		}
		if (value == null && null != map) {
			value = Double.toString(map.getInitLatitude());
			step = map.getInitStep();
		} else {
			value = Double.toString(getInitLatitude());
			step = getInitStep();
		}
		String value2 = request.getParameter(prop2.getName());
		if (value2 == null)
			value2 = ob.getProperty(prop2);
		if (value2 == null && null != map)
			value2 = Double.toString(map.getInitLongitude());
		else
			value2 = Double.toString(getInitLongitude());
		/** Class Cast Exception in value = obj.getProperty(prop). */
