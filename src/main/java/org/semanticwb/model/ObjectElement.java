	...
	public String renderElement(HttpServletRequest request, SemanticObject obj, SemanticProperty prop, String propName,
		...
		if (prop.isObjectProperty()) {
			if (prop.getCardinality() != 1) {
				...
				url += "?scls=" + cls2.getEncodedURI() + "&sref=" + obj.getEncodedURI() + "&sprop="
						+ prop.getEncodedURI() + "&chcls=true";
				...
				while (it.hasNext()) {
					...
					ret.append(
							"<div style=\"background-color:#FFFFFF; cursor:text; font:11px; width:400px; border:#A0A0FF solid 1px;\">");
					ret.append("<a href=\"?suri=" + value.getEncodedURI() + "\" onclick=\"addNewTab('" + value.getURI()
							+ "', null, '" + value.getDisplayName(lang) + "');return false;\">" + value.getDisplayName()
							+ "</a>");
					ret.append("</div>");
					ret.append("<br/>");
				}
