...
public class ImageUpload extends org.semanticwb.model.base.ImageUploadBase {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.semanticwb.model.FileUpload#renderElement(HttpServletRequest,
	 * SemanticObject, SemanticProperty, String, String, String)
	 */
	@Override
	public String renderElement(HttpServletRequest request, SemanticObject obj, SemanticProperty prop, String propName,
		...
		if (!"view".equals(mode)) {
			buffer.append("<input " + "name=\"uploadedfile\" " + "data-dojo-props=\" \n" + "multiple:'"
					+ (prop.getCardinality() != 1 ? "true" : "false") + "', \n" + "uploadOnSelect:'true', \n" + "url:'"
					...
					+ "data-dojo-type=\"dojox.form.Uploader\" " + "label=\"Select File\" " + "id=\"" + pname
					+ "_defaultAuto\" " + "/>  ");
			buffer.append("<br/>\n");
				...
				if (prop.getCardinality() == 1) {
					if ("edit".equals(mode)) {
						buffer.append("Eliminar: <input dojoType=\"dijit.form.CheckBox\" id=\"" + pname
								...
								+ SWBPlatform.getContextPath() + "/work" + obj.getWorkPath() + "/"
								+ obj.getProperty(prop) + "\">" + name + "</a>\n");
					} else {
				...
				if (prop.getCardinality() == 1) {
					...
					if (lista.hasNext()) {
						while (lista.hasNext()) {
							...
							buffer.append("&nbsp;<a href=\"" + SWBPlatform.getContextPath() + "/work"
									+ obj.getWorkPath() + "/" + fname + "\">" + name + "</a>");
						}
					}
				}
			}
	...
	public String renderElement_(HttpServletRequest request, SemanticObject obj, SemanticProperty prop, String propName,
			String type, String mode, String lang) {
		...
		String url = SWBPlatform.getContextPath() + "/multiuploader/" + obj.getModel().getModelObject().getId() + "/"
				+ page + "/" + cad;
		String enviar = lang.equals("en") ? "You have to send the selected files first"
				: "Debe enviar primero los archivos seleccionados";
		String eliminar = lang.equals("en") ? "Chose the files to delete" : "Selecione el(los) archivo(s) a eliminar";
