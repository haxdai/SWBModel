...
/**
 * Elemento usado para validar la posibilidad de ser o no activado del elemento,
 * ejemplo: cuando el elmenento esta en flujo no puede ser activado
 */
public class Activate extends org.semanticwb.model.base.ActivateBase {
	...
	public String renderElement(HttpServletRequest request, SemanticObject obj, SemanticProperty prop, String propName,
		...
		if (so.instanceOf(Resource.sclass)) {
			Resource res = (Resource) so.createGenericInstance();
		...
		if (prop.isDataTypeProperty() && prop.isBoolean()) {
			...
			if (aux != null) {
				...
				value = so.getBooleanProperty(prop);
			}
			...
			if (!mode.equals("view") && disabled && value) {
	...
	public boolean isInFlow(Resource resource) {
		if (resource.getPflowInstance() == null) {
			...
			if (!(resource.getPflowInstance().getStatus() == 3 || resource.getPflowInstance().getStatus() == 2
					|| resource.getPflowInstance().getStep() == null)) {
				return true;
			}
		}
		return false;
